...
{else}
	...
	<div id="order-detail-content" class="table_block table-responsive col-md-8">
		<div class="box">
		<table id="cart_summary" class="table table-hover{if $PS_STOCK_MANAGEMENT} stock-management-on{else} stock-management-off{/if}">
			<thead>
				<tr>
										...
										<div class="cart_quantity_button clearfix">
											{if $product.minimal_quantity < ($customization.quantity -$quantityDisplayed) OR $product.minimal_quantity <= 1}
												<a
													...
													rel="nofollow"
													title="{l s='Subtract'}">
													<span><i class="icon icon-fw icon-minus"></i></span>
												</a>
											{else}
												<a
											...
											<a
												...
												rel="nofollow"
												title="{l s='Add'}">
												<span><i class="icon icon-fw icon-plus"></i></span>
											</a>
										</div>
									{/if}
								</td>
	...
	<div class="order-detail-summary table-responsive col-md-4">
...
{* 						<input type="text" class="discount_name form-control" id="discount_name" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
		...
		{* 			</fieldset> *}
					...
					<div class="input-group">
					      <input type="text" class="discount_name form-control" id="discount_name" name="discount_name" placeholder="{l s='Inset your voucher code here'}">
					      <span class="input-group-btn">
					        <button class="btn btn-default" type="submit" name="submitAddDiscount" >{l s='Add'}</button>
					      </span>
					</div>
				</form>
				{if $displayVouchers}
...
{* 							<tr id="cart_voucher" class="cart_voucher">
								<td colspan="5">
									{if $voucherAllowed}
										<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
											<fieldset>
...
{* 								<td rowspan="{$rowspan_total}" colspan="2" id="cart_voucher" class="cart_voucher">
	...
	<p class="cart_navigation clearfix">
		{if !$opc}
			<a  href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')|escape:'html':'UTF-8'}{else}{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}{/if}" class="btn btn-default standard-checkout" title="{l s='Proceed to checkout'}">
				<span>{l s='Proceed to checkout'}<i class="icon icon-fw icon-fw icon-fw icon-chevron-right"></i></span>
			</a>
		{/if}
		<a href="{if (isset($smarty.server.HTTP_REFERER) && ($smarty.server.HTTP_REFERER == $link->getPageLink('order', true) || $smarty.server.HTTP_REFERER == $link->getPageLink('order-opc', true) || strstr($smarty.server.HTTP_REFERER, 'step='))) || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'html':'UTF-8'|secureReferrer}{/if}" class="btn btn-default" title="{l s='Continue shopping'}">
			<i class="icon icon-fw icon-fw icon-fw icon-chevron-left"></i>{l s='Continue shopping'}
		</a>
	</p>
	<div class="clear"></div>
